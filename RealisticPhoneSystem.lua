...
function RealisticPhone.new()
    ...
    print("📱 Realistic Phone System initialized successfully!")
    print("📱 Toggle button should be visible in the top-right area!")
...
function RealisticPhone:createRealisticGUI()
    ...
    -- TOGGLE BUTTON (Floating) - Make sure it's created
    self:createToggleButton()
    
    -- Verify toggle button was created
    if not self.toggleButton then
        warn("❌ Toggle button failed to create! Creating fallback...")
...
-- Create Toggle Button (Right Center Position) - SIMPLIFIED VERSION
function RealisticPhone:createToggleButton()
    -- Simple toggle button that will definitely appear
    local toggleButton = Instance.new("TextButton")
    toggleButton.Name = "PhoneToggle"
    toggleButton.Size = UDim2.new(0, 80 * self.scale, 0, 80 * self.scale)
    toggleButton.Position = UDim2.new(1, -100 * self.scale, 0.5, -40 * self.scale)
    ...
    statusDot.Size = UDim2.new(0, 15 * self.scale, 0, 15 * self.scale)
    statusDot.Position = UDim2.new(1, -20 * self.scale, 0, 5 * self.scale)
    ...
    statusText.Size = UDim2.new(1, 0, 0, 20 * self.scale)
    ...
    toggleButton.MouseEnter:Connect(function()
        TweenService:Create(toggleButton, TweenInfo.new(0.2), 
            {Size = UDim2.new(0, 90 * self.scale, 0, 90 * self.scale)}):Play()
    end)
    ...
    spawn(function()
        while toggleButton and toggleButton.Parent do
            TweenService:Create(toggleButton, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), 
                {Size = UDim2.new(0, 85 * self.scale, 0, 85 * self.scale)}):Play()
            wait(1)
            TweenService:Create(toggleButton, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), 
                {Size = UDim2.new(0, 80 * self.scale, 0, 80 * self.scale)}):Play()
            wait(1)
...
-- Create Fallback Toggle Button (Emergency backup)
function RealisticPhone:createFallbackToggleButton()
    print("🆘 Creating fallback toggle button...")
    
    local fallbackButton = Instance.new("TextButton")
    fallbackButton.Name = "FallbackPhoneToggle"
    fallbackButton.Size = UDim2.new(0, 100, 0, 100)
    fallbackButton.Position = UDim2.new(1, -120, 0.5, -50)
    fallbackButton.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
    fallbackButton.BorderSizePixel = 0
    fallbackButton.Text = "📱"
    fallbackButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    fallbackButton.TextScaled = true
    fallbackButton.Font = Enum.Font.GothamBold
    fallbackButton.ZIndex = 2000
    fallbackButton.Parent = self.screenGui
    
    local fallbackCorner = Instance.new("UICorner")
    fallbackCorner.CornerRadius = UDim.new(1, 0)
    fallbackCorner.Parent = fallbackButton
    ...
    self.toggleButton = fallbackButton
    print("✅ Fallback toggle button created!")
end
...
function RealisticPhone:updateToggleButtonStatus()
    if self.statusDot and self.statusText then
        if self.isPhoneOpen then
            self.statusDot.BackgroundColor3 = Color3.fromRGB(52, 199, 89) -- Green
            self.statusText.Text = "OPEN"
            self.statusText.TextColor3 = Color3.fromRGB(52, 199, 89)
        else
            self.statusDot.BackgroundColor3 = Color3.fromRGB(255, 59, 48) -- Red
...
if realisticPhone then
    ...
    print("")
    print("🔍 DEBUG INFO:")
    print("  📱 Toggle button should be visible in top-right")
    print("  📱 If not visible, look for red fallback button")
    print("  📱 Check console for toggle button creation messages")
    print("")
